...
int main(int argc,char *argv[]){
	...
	qualifier=qs;
	string olddatafilename[6]={"bf_phi.txt","bf_eta.txt","bf_y.txt","bf_eta1.txt","bf_y1.txt","bf_qinv.txt"};
	string newdatafilename[6]={"bf_phi.txt","bf_eta.txt","bf_y.txt","bf_eta1.txt","bf_y1.txt","bf_qinv.txt"};
	...
	for(jdir=0;jdir<NJDIR;jdir++){
		oldfilename=dirprefix+to_string(jdir)+"/"+qualifier+"/"+acceptance+"/denom.txt";
	...
	newfilename="sum/"+qualifier+"/"+acceptance+"/denom.txt";
	printf("newfilename for denom=%s\n",newfilename.c_str());
	...
	for(itype=0;itype<NTYPES;itype++){
		...
		for(ifn=0;ifn<Nqcut;ifn++){
			...
			if(ifn==5)
				dx=10.0;
			newdirname="sum/"+qualifier+"/"+acceptance+"/"+dirname[itype];
			...
			ysum[3].clear();
			for(jdir=0;jdir<NJDIR;jdir++){
				oldfilename=dirprefix+to_string(jdir)+"/"+qualifier+"/"+acceptance+"/"+dirname[itype]+"/"+olddatafilename[ifn];
				printf("oldfilename=%s\n",oldfilename.c_str());
				fptr_read=fopen(oldfilename.c_str(),"r");
				ix=0;
